	...
	return async (tree, file) => {
		...
		if (validationComponents) {
			try {
				...
				documentType = validationComponents.documentTypeDetector.detectType(file.path || "", content, documentStructure)
				validationRules = validationComponents.validationRuleConfig.getRulesForDocument(file.path || "", content, documentStructure)
			} catch (error) {
		...
		const contextAwareSettings = {
			...settings,
			// Override settings based on document type
			requireResearchContext: validationRules.requireResearchContext?.value ?? settings.requireResearchContext,
			requireNavigationFooter: validationRules.requireNavigationFooter?.value ?? settings.requireNavigationFooter,
			requireNoDeadEndsPolicy: validationRules.requireNoDeadEndsPolicy?.value ?? settings.requireNoDeadEndsPolicy,
			requireWhenYoureHere: validationRules.requireWhenYoureHere?.value ?? settings.requireWhenYoureHere,
			contentQualityThreshold: validationRules.contentQualityThreshold?.value ?? settings.contentQualityThreshold,
			maxLineLength: validationRules.maxLineLength?.value ?? settings.maxLineLength,
			minSectionLength: validationRules.minSectionLength?.value ?? settings.minSectionLength,
		}
		...
		// Validate comprehensive standards with context-aware settings
		validateComprehensiveStandards(documentStructure, issues, warnings, contextAwareSettings, file)

		// Validate content quality with context-aware settings
		validateContentQuality(metrics, issues, warnings, contextAwareSettings, file)
			...
			try {
				...
				for (const crossRef of documentStructure.crossReferences) {
					try {
						...
						if (!result.valid && !result.isTemplate) {
...
function validateTableOfContentsLinks(structure, issues, file) {
	...
	const headingIds = structure.headings.map((heading) => {
		// Enhanced heading to anchor conversion that handles emojis and special characters
		return heading.text
			.toLowerCase()
	...
	for (const link of tocLinks) {
		...
		// Try fuzzy matching for common patterns
		const fuzzyMatch = headingIds.find((id) => 
			id.includes(targetId) || 
			targetId.includes(id) ||
			id.replace(/-/g, "") === targetId.replace(/-/g, "")
		)

		if (fuzzyMatch) {
			// Found a fuzzy match, suggest the correct link
			issues.push({
				type: "warning",
				message: `Table of Contents link "${link.url}" should be "#${fuzzyMatch}"`,
				line: link.line,
				column: 1,
				rule: "kilocode-toc-link-mismatch",
				suggestion: `Update link to "#${fuzzyMatch}"`,
			})
			continue
		}
		...
		if (commonFalsePositives.includes(targetId)) {
			// This is a known false positive, skip it
			continue
		}
...
function validateNavigationConsistency(structure, issues, warnings, file) {
	...
	// Check for consistent navigation format (more flexible)
	const hasConsistentFormat = navigationLinks.every((link) => {
		const text = link.text
		// Accept various navigation patterns
	...
	// Only warn if there are many navigation links and they're inconsistent
	if (navigationLinks.length > 3 && !hasConsistentFormat) {
		warnings.push({
			type: "warning",
	...
	// Only require glossary link for documents with many sections
	if (structure.headings.length >= 5 && !hasGlossaryLink) {
		warnings.push({
			type: "warning",
	...
	// Only require TOC link for documents with many sections
	if (structure.headings.length >= 5 && !hasTOCLink) {
		warnings.push({
			type: "warning",
 ...
 * Validate required sections for document type
 */
function validateRequiredSections(structure, issues, warnings, file, relativePath, detectedDocumentType = null) {
	// Use detected document type or fall back to path-based detection
	let documentType = detectedDocumentType || "general"
