/**
 * Remark KiloCode Standards Plugin
 *
 * Custom validation plugin for KiloCode documentation standards.
 * Enforces KiloCode-specific documentation rules and conventions.
 */

import { visit } from "unist-util-visit"

/**
 * KiloCode Standards Validation Plugin
 *
 * Validates documentation against KiloCode-specific standards including:
 * - Research Context section presence
 * - Navigation footer requirements
 * - Fun fact presence (warnings)
...
function remarkKiloCodeStandards(options = {}) {
	...
	return (tree, file) => {
		...
		visit(tree, (node, index, parent) => {
			...
			// Check for dead ends (documents with no outgoing links)
			if (node.type === "paragraph" || node.type === "list") {
				const text = getNodeText(node)
				if (text.length > 100 && !hasOutgoingLinks(node)) {
					documentStructure.deadEnds.push({
						line: node.position?.start?.line || 0,
		...
		// Validate no dead ends policy
		if (settings.enforceNoDeadEnds) {
			validateNoDeadEnds(documentStructure, issues, file)
		}
...
function validateDocumentStructure(structure, issues, warnings, settings, file) {
	...
	if (settings.warnOnMissingFunFacts && shouldHaveFunFact(relativePath) && !structure.hasFunFact) {
		warnings.push({
			type: "warning",
 ...
 * Validate no dead ends policy
 */
function validateNoDeadEnds(structure, issues, file) {
	if (structure.deadEnds.length > 0 && structure.links.length === 0) {
		issues.push({
			type: "warning",
...
export default remarkKiloCodeStandards
