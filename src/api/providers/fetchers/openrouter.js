...
import { z } from "zod";
import { isModelParameter, OPEN_ROUTER_COMPUTER_USE_MODELS, OPEN_ROUTER_REASONING_BUDGET_MODELS, OPEN_ROUTER_REQUIRED_REASONING_BUDGET_MODELS, anthropicModels, } from "@roo-code/types";
...
export async function getOpenRouterModels(options) {
    ...
    try {
        ...
        if (!result.success) {
            ...
            throw new Error("OpenRouter models response is invalid: " + JSON.stringify(result.error.format(), undefined, 2));
            // kilocode_change end
        }
 ...
 */
export const parseOpenRouterModel = ({ id, model, displayName, // kilocode_change
inputModality, outputModality, maxTokens, supportedParameters, }) => {
