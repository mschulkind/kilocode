...
// kilocode_change end
export async function getCheckpointService(task, { interval = 250, timeout = 15_000 } = {}) {
...
export async function checkpointDiff(task, { ts, previousCommitHash, commitHash, mode }) {
    ...
    try {
        ...
        await vscode.commands.executeCommand("vscode.changes", mode === "full" ? "Changes since task started" : "Changes compare with next checkpoint", changes.map((change) => [
