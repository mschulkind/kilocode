...
export async function getCommitRangeForNewCompletion(task) {
    try {
        ...
        const messages = task.clineMessages.at(-1)?.say === "completion_result"
        ...
        const previousCompletionIndex = findLast(messages, (msg) => msg.type === "say" && msg.say === "completion_result");
        ...
        if (lastCheckpointIndex >= 0 && previousCompletionIndex >= 0 && lastCheckpointIndex < previousCompletionIndex) {
            console.log(`getCommitRangeForNewCompletion: last checkpoint ${lastCheckpointIndex} is older than previous completion ${previousCompletionIndex}.`);
            return undefined;
        }
        const previousCheckpointIndex = previousCompletionIndex >= 0
            ? findLast(messages.slice(0, previousCompletionIndex), (msg) => msg.type === "say" && msg.say === "checkpoint_saved")
            : -1;
...
export async function seeNewChanges(task, commitRange) {
    try {
        const service = await getCheckpointService(task);
        if (!service) {
        ...
        await vscode.commands.executeCommand("vscode.changes", t("kilocode:seeNewChanges.title"), changes.map((change) => [
