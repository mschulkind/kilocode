...
export async function processKiloUserContentMentions({ context, // kilocode_change
userContent, cwd, urlContentFetcher, fileContextTracker, rooIgnoreController, showRooIgnoredFiles = true, includeDiagnosticMessages = true, maxDiagnosticMessages = 50, maxReadFileLine, }) {
        ...
        return await Promise.all(userContent.map(async (block) => {
            ...
            if (block.type === "text") {
                if (shouldProcessMentions(block.text)) {
                    ...
                    const parsedText = await parseMentions(block.text, cwd, urlContentFetcher, fileContextTracker, rooIgnoreController, showRooIgnoredFiles, includeDiagnosticMessages, maxDiagnosticMessages, maxReadFileLine);
                    // when parsing slash commands, we still want to allow the user to provide their desired context
                    const { processedText, needsRulesFileCheck: needsCheck } = await parseKiloSlashCommands(parsedText, localWorkflowToggles, // kilocode_change
                ...
                if (typeof block.content === "string") {
                    if (shouldProcessMentions(block.content)) {
                        return {
                            ...block,
                            content: await parseMentions(block.content, cwd, urlContentFetcher, fileContextTracker, rooIgnoreController, showRooIgnoredFiles, includeDiagnosticMessages, maxDiagnosticMessages, maxReadFileLine),
                        };
                    }
                    return block;
                }
                    ...
                    const parsedContent = await Promise.all(block.content.map(async (contentBlock) => {
                        if (contentBlock.type === "text" && shouldProcessMentions(contentBlock.text)) {
                            return {
                                ...contentBlock,
                                text: await parseMentions(contentBlock.text, cwd, urlContentFetcher, fileContextTracker, rooIgnoreController, showRooIgnoredFiles, includeDiagnosticMessages, maxDiagnosticMessages, maxReadFileLine),
                            };
                        }
                        return contentBlock;
                    }));
...
//# sourceMappingURL=processKiloUserContentMentions.js.map