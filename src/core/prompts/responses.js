...
export const formatResponse = {
    ...
    rooIgnoreError: (path) => `Access to ${path} is blocked by the .kilocodeignore file settings. You must try to continue in the task without using this file, or ask the user to update the .kilocodeignore file.`,
...
(This is an automated message, so do not respond to it conversationally.)`,
    ...
    lineCountTruncationError: (actualLineCount, isNewFile, diffStrategyEnabled = false) => {
        ...
        existingFileApproaches.push(`${diffStrategyEnabled ? "3" : "2"}. Or use search_and_replace for specific text replacements`, `${diffStrategyEnabled ? "4" : "3"}. Or use insert_content to add specific content at particular lines`);
...
const formatImagesIntoBlocks = (images) => {
    return images
        ? images.map((dataUrl) => {
            ...
            const mimeType = rest?.split(":")[1]?.split(";")[0] || "image/png";
            return {
                type: "image",
