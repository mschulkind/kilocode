    ...
    for (const file of ruleFiles) {
        ...
        if (content) {
            if (file !== ".kilocoderules" && vscodeAPI && !hasShownNonKilocodeRulesMessage) {
                ...
                vscodeAPI.window.showWarningMessage(`Loading non-Kilocode rules from ${file}, consider moving to .kilocode/rules/`);
...
export async function addCustomInstructions(modeCustomInstructions, globalCustomInstructions, cwd, mode, 
...
options = {}) {
    ...
    if (hasAnyToggles(options.localRulesToggleState) || hasAnyToggles(options.globalRulesToggleState)) {
        const genericRuleContent = (await loadEnabledRules(cwd, options.localRulesToggleState || {}, options.globalRulesToggleState || {}, directoryExists, readTextFilesFromDirectory))?.trim() ?? "";
