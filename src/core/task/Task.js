...
import { presentAssistantMessage } from "../assistant-message/index";
import { AssistantMessageParser } from "../assistant-message/AssistantMessageParser";
import { truncateConversationIfNeeded } from "../sliding-window/index";
import { MultiSearchReplaceDiffStrategy } from "../diff/strategies/multi-search-replace";
import { MultiFileSearchReplaceDiffStrategy } from "../diff/strategies/multi-file-search-replace";
import { readApiMessages, saveApiMessages, readTaskMessages, saveTaskMessages, taskMetadata, } from "../task-persistence/index";
import { getEnvironmentDetails } from "../environment/getEnvironmentDetails";
import { getCheckpointService, checkpointSave, checkpointRestore, checkpointDiff, } from "../checkpoints/index";
    ...
    async *attemptApiRequest(retryAttempt = 0) {
        ...
        if (Task.lastGlobalApiRequestTime) {
            ...
            if (rateLimitDelay > rateLimit) {
                console.warn(`rateLimitDelay ${rateLimitDelay}s is larger than the configured rateLimit ${rateLimit}s; this makes no sense`);
        ...
        try {
            ...
            if (autoApprovalEnabled && alwaysApproveResubmit) {
                ...
                const { response } = await this.ask("api_req_failed", error instanceof Error ? error.message : JSON.stringify(serializeError(error), null, 2));
