...
describe("runSlashCommandTool", () => {
    ...
    it("should handle command not found", async () => {
        ...
        await runSlashCommandTool(mockTask, block, mockAskApproval, mockHandleError, mockPushToolResult, mockRemoveClosingTag);
        expect(mockTask.recordToolError).toHaveBeenCalledWith("run_slash_command");
        expect(mockPushToolResult).toHaveBeenCalledWith(formatResponse.toolError("Command 'nonexistent' not found. Available commands: init, test, deploy"));
    });
    ...
    it("should successfully execute built-in command", async () => {
        ...
        vi.mocked(getCommand).mockResolvedValue(mockCommand);
        await runSlashCommandTool(mockTask, block, mockAskApproval, mockHandleError, mockPushToolResult, mockRemoveClosingTag);
        expect(mockAskApproval).toHaveBeenCalledWith("tool", JSON.stringify({
        ...
        expect(mockPushToolResult).toHaveBeenCalledWith(`Command: /init
Description: Analyze codebase and create AGENTS.md
Source: built-in
...
Initialize project content here`);
    ...
    it("should successfully execute command with arguments", async () => {
        ...
        vi.mocked(getCommand).mockResolvedValue(mockCommand);
        await runSlashCommandTool(mockTask, block, mockAskApproval, mockHandleError, mockPushToolResult, mockRemoveClosingTag);
        expect(mockPushToolResult).toHaveBeenCalledWith(`Command: /test
Description: Run project tests
...
Run tests with specific focus`);
    ...
    it("should handle global command", async () => {
        ...
        vi.mocked(getCommand).mockResolvedValue(mockCommand);
        await runSlashCommandTool(mockTask, block, mockAskApproval, mockHandleError, mockPushToolResult, mockRemoveClosingTag);
        expect(mockPushToolResult).toHaveBeenCalledWith(`Command: /deploy
Source: global
