...
// kilocode_change end
export async function attemptCompletionTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag, toolDescription, askFinishSubTaskApproval) {
        ...
        if (block.partial) {
            if (command) {
                ...
                if (lastMessage && lastMessage.ask === "command") {
                    ...
                    await cline.say("completion_result", removeClosingTag("result", result), undefined, false, undefined, undefined, await getClineMessageOptions(cline));
            ...
            await cline.say("completion_result", result, undefined, false, undefined, undefined, await getClineMessageOptions(cline));
            TelemetryService.instance.captureTaskCompleted(cline.taskId);
            cline.emit(RooCodeEventName.TaskCompleted, cline.taskId, cline.getTokenUsage(), cline.toolUsage);
            if (cline.parentTask) {
