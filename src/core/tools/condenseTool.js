...
export const condenseTool = async (cline, block, askApproval, handleError, pushToolResult, removeClosingTag) => {
    ...
    try {
        if (block.partial) {
            ...
            if (text || images?.length) {
                ...
                const summarizedMessages = await summarizeConversation(cline.apiConversationHistory, cline.api, await cline.getSystemPrompt(), "TaskId condenseTool", prevContextTokens);
