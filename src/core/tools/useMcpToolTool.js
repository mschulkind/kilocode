...
async function handlePartialRequest(cline, params, removeClosingTag) {
...
async function validateParams(cline, params, pushToolResult) {
...
async function validateToolExists(cline, serverName, toolName, pushToolResult) {
    try {
        ...
        if (!server.tools || server.tools.length === 0) {
            ...
            cline.consecutiveMistakeCount++;
            cline.recordToolError("use_mcp_tool");
            await cline.say("error", t("mcp:errors.toolNotFound", {
                toolName,
                serverName,
        ...
        if (tool.enabledForPrompt === false) {
            ...
            cline.consecutiveMistakeCount++;
            cline.recordToolError("use_mcp_tool");
            await cline.say("error", t("mcp:errors.toolDisabled", {
                toolName,
                serverName,
...
async function sendExecutionStatus(cline, status) {
...
async function processToolContent(task, toolResult) {
...
async function executeToolAndProcessResult(cline, serverName, toolName, parsedArguments, executionId, pushToolResult) {
