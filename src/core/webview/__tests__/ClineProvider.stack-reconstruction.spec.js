...
describe("Orchestrator Stack Reconstruction", () => {
    ...
    describe("reconstructTaskStack", () => {
            ...
            vi.spyOn(provider, "getTaskWithId").mockImplementation(async (id) => {
                switch (id) {
                    case "orchestrator-root":
                        return { historyItem: rootTaskHistoryItem };
                    case "code-subtask":
                        return { historyItem: codeSubtaskHistoryItem };
                    case "debug-subtask":
                        return { historyItem: debugSubtaskHistoryItem };
                    default:
        ...
        it("should handle simple parent-child relationship", async () => {
            ...
            vi.spyOn(provider, "getTaskWithId").mockImplementation(async (id) => {
                switch (id) {
                    case "parent-task":
                        return { historyItem: parentTaskHistoryItem };
                    case "child-task":
                        return { historyItem: childTaskHistoryItem };
                    default:
                        throw new Error(`Task not found: ${id}`);
                }
            });
        ...
        it("should handle standalone tasks normally", async () => {
            const standaloneTaskHistoryItem = {
                id: "standalone-task",
                rootTaskId: undefined,
                parentTaskId: undefined,
                number: 1,
            ...
            const createTaskSpy = vi.spyOn(provider, "createTaskWithHistoryItem").mockResolvedValue({});
            // Mock postMessageToWebview
            vi.spyOn(provider, "postMessageToWebview").mockResolvedValue();
    ...
    describe("buildTaskHierarchy", () => {
        it("should build correct hierarchy for nested subtasks", async () => {
            ...
            const intermediateTaskHistoryItem = {
            ...
            vi.spyOn(provider, "getTaskWithId").mockImplementation(async (id) => {
                switch (id) {
                    ...
                    case "intermediate-task":
                        return { historyItem: intermediateTaskHistoryItem };
                    case "target-task":
                        return { historyItem: targetTaskHistoryItem };
                    default:
                        throw new Error(`Task not found: ${id}`);
                }
            });
            // Call the private method using type assertion
            const hierarchy = await provider.buildTaskHierarchy(targetTaskHistoryItem);
        ...
        it("should handle circular references gracefully", async () => {
            ...
            vi.spyOn(provider, "getTaskWithId").mockImplementation(async (id) => {
                switch (id) {
                    case "task1":
                        return { historyItem: task1HistoryItem };
                    case "task2":
                        return { historyItem: task2HistoryItem };
                    default:
                        throw new Error(`Task not found: ${id}`);
                }
            });
            // Call the private method - should not hang due to circular reference
            const hierarchy = await provider.buildTaskHierarchy(task1HistoryItem);
            ...
            vi.spyOn(provider, "getTaskWithId").mockImplementation(async (id) => {
                switch (id) {
                    case "orchestrator-main":
                        return { historyItem: orchestratorHistoryItem };
                    case "code-subtask":
                        return { historyItem: subtaskHistoryItem };
                    default:
                        throw new Error(`Task not found: ${id}`);
                }
            });
...
//# sourceMappingURL=ClineProvider.stack-reconstruction.spec.js.map