    ...
    describe("readUserConfigFile", () => {
        ...
        it("should handle different editor installations", async () => {
            ...
            for (const { appName, expectedCommand } of testCases) {
                ...
                vi.mocked(fs.stat).mockResolvedValue({ isFile: () => true, mtimeMs: Date.now() });
                vi.mocked(vscode.workspace.fs.readFile).mockResolvedValue(Buffer.from(JSON.stringify([{ key: "cmd+i", command: expectedCommand }]), "utf8"));
                const result = await readUserConfigFile("keybindings.json");
        ...
        it("should handle portable installation", async () => {
            ...
            vi.mocked(fs.readdir).mockRejectedValue(new Error("No profiles"));
            vi.mocked(fs.stat).mockResolvedValue({ isFile: () => true, mtimeMs: Date.now() });
            vi.mocked(vscode.workspace.fs.readFile).mockResolvedValue(Buffer.from(JSON.stringify([{ key: "f1", command: "portable.command" }]), "utf8"));
            const result = await readUserConfigFile("keybindings.json");
        ...
        it("should handle code-server environment", async () => {
            ...
            vi.mocked(fs.readdir).mockRejectedValue(new Error("No profiles"));
            vi.mocked(fs.stat).mockResolvedValue({ isFile: () => true, mtimeMs: Date.now() });
            vi.mocked(vscode.workspace.fs.readFile).mockResolvedValue(Buffer.from(JSON.stringify([{ key: "ctrl+`", command: "terminal.new" }]), "utf8"));
            const result = await readUserConfigFile("keybindings.json");
        ...
        it("should handle profiles and prefer most recent", async () => {
            ...
            vi.mocked(fs.readdir).mockResolvedValue(mockProfiles);
            vi.mocked(fs.stat)
            ...
            vi.mocked(vscode.workspace.fs.readFile).mockResolvedValueOnce(Buffer.from(JSON.stringify([{ key: "cmd+p", command: "profile2.command" }]), "utf8"));
            const result = await readUserConfigFile("keybindings.json");
