...
export interface DocumentTypeConfig {
  types: Record<string, DocumentTypeDefinition>
  fallback: string
}
...
export type DocumentType = 'navigation' | 'technical' | 'planning' | 'general'

export class DocumentTypeDetector {
  private config: DocumentTypeConfig

  constructor(config?: Partial<DocumentTypeConfig>) {
  ...
  detectType(filePath: string, content: string, structure: string): DocumentTypeResult {
    const results: Array<{ type: string; confidence: number; reason: string }> = []
    ...
    const typeGroups = new Map<string, Array<{ confidence: number; reason: string }>>()
  ...
  getTypeInfo(type: string): TypeInfo | null {
    ...
    const typeInfos: Record<string, TypeInfo> = {
      navigation: {
   ...
   */
  updateConfig(newConfig: DocumentTypeConfig): void {
   ...
   */
  private detectByPath(filePath: string): { type: string; confidence: number; reason: string } | null {
   ...
   */
  private detectByContent(content: string): { type: string; confidence: number; reason: string } | null {
   ...
   */
  private detectByStructure(content: string): { type: string; confidence: number; reason: string } | null {
    if (!content) return null
   ...
   */
  private matchesStructurePattern(content: string, pattern: string): boolean {
    const regex = new RegExp(pattern, 'i')
    return regex.test(content)
  }
   ...
   */
  private mergeWithDefaults(config: Partial<DocumentTypeConfig>): DocumentTypeConfig {
    const defaultConfig: DocumentTypeConfig = {
      types: {
        navigation: {
          name: 'Navigation Document',
          pathPatterns: [
   ...
   */
  private validateConfig(config: DocumentTypeConfig): void {
    if (!config.types || Object.keys(config.types).length === 0) {
