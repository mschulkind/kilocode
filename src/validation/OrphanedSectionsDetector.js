...
export class OrphanedSectionsDetector {
     ...
     */
    detectOrphanedSections(filePath, content) {
        const result = {
            orphanedSections: [],
            totalSections: 0,
     ...
     */
    analyzeSection(filePath, title, content, level) {
     ...
     */
    detectReferences(filePath, content) {
        const references = [];
        const lines = content.split("\n");
     ...
     */
    getSectionStats(filePath, content) {
        const sections = this.parseSections(content);
        if (sections.length === 0) {
            return {
                totalSections: 0,
     ...
     */
    isSectionReferenced(sectionTitle, references) {
        ...
        return references.some((ref) => {
            ...
            return (normalizedTarget.includes(normalizedTitle) ||
     ...
     */
    calculateConnectivityScore(section, allSections, index) {
        let score = 0;
     ...
     */
    generateReasons(analysis, isReferenced, connectivityScore) {
        const reasons = [];
     ...
     */
    findLineInContent(filePath, title) {
