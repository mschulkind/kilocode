...
describe('FileIndexBuilder', () => {
    ...
    describe('buildIndex', () => {
        it('should discover markdown files recursively', async () => {
            ...
            mockFs.readdir
                .mockResolvedValueOnce(['docs', 'README.md', 'other.txt'])
                .mockResolvedValueOnce(['guide.md', 'tutorial.md'])
                .mockResolvedValueOnce([]); // Empty subdirectory
            mockFs.stat
                .mockResolvedValueOnce({ isDirectory: () => true }) // docs/
                .mockResolvedValueOnce({ isDirectory: () => false, isFile: () => true, mtime: new Date('2023-01-01') }) // README.md
                .mockResolvedValueOnce({ isDirectory: () => false, isFile: () => true, mtime: new Date('2023-01-02') }) // other.txt
                .mockResolvedValueOnce({ isDirectory: () => false, isFile: () => true, mtime: new Date('2023-01-03') }) // guide.md
                .mockResolvedValueOnce({ isDirectory: () => false, isFile: () => true, mtime: new Date('2023-01-04') }) // tutorial.md
                .mockResolvedValueOnce({ isDirectory: () => true }); // Empty subdirectory
        ...
        it('should respect exclude patterns', async () => {
            ...
            mockFs.readdir
                .mockResolvedValueOnce(['docs', 'temp', 'node_modules'])
                .mockResolvedValueOnce(['file.md']);
            mockFs.stat
                .mockResolvedValueOnce({ isDirectory: () => true }) // docs/
                .mockResolvedValueOnce({ isDirectory: () => true }) // temp/
                .mockResolvedValueOnce({ isDirectory: () => true }) // node_modules/
                .mockResolvedValueOnce({ isDirectory: () => false, isFile: () => true, mtime: new Date(), size: 1000 }); // file.md
        ...
        it('should respect max depth limit', async () => {
            ...
            mockFs.stat
                .mockResolvedValueOnce({ isDirectory: () => true }) // level1/
                .mockResolvedValueOnce({ isDirectory: () => true }); // level2/
        ...
        it('should handle permission errors gracefully', async () => {
            const rootPath = '/project';
            mockFs.readdir
                .mockResolvedValueOnce(['docs', 'restricted'])
                .mockResolvedValueOnce(['file.md'])
                .mockRejectedValueOnce(new Error('EACCES: permission denied')); // restricted/
            mockFs.stat
                .mockResolvedValueOnce({ isDirectory: () => true }) // docs/
                .mockResolvedValueOnce({ isDirectory: () => true }) // restricted/
                .mockResolvedValueOnce({ isDirectory: () => false, isFile: () => true, mtime: new Date() }); // file.md
            const result = await indexBuilder.buildIndex(rootPath);
            expect(result.files).toHaveLength(1);
            expect(result.files[0].path).toBe('/project/docs/file.md');
        ...
        it('should track file metadata', async () => {
            ...
            mockFs.readdir.mockResolvedValueOnce(['file.md']);
    ...
    describe('incremental updates', () => {
        ...
        it('should detect new files', async () => {
            const rootPath = '/project';
            // Initial build
            mockFs.readdir.mockResolvedValueOnce(['file1.md']);
            mockFs.stat.mockResolvedValueOnce({
                isDirectory: () => false,
                isFile: () => true,
                mtime: new Date(),
    ...
    describe('cache management', () => {
        ...
        it('should expire cache entries after TTL', async () => {
            ...
            const rootPath = '/project';
            mockFs.readdir.mockResolvedValue(['file.md']);
    ...
    describe('file lookup', () => {
        beforeEach(async () => {
            ...
            mockFs.stat
                ...
                .mockResolvedValueOnce({ isDirectory: () => false, isFile: () => true, mtime: new Date(), size: 1000 })
                .mockResolvedValueOnce({ isDirectory: () => false, isFile: () => true, mtime: new Date(), size: 2000 });
            await indexBuilder.buildIndex(rootPath);
        });
    ...
    describe('performance and memory', () => {
        it('should handle large file sets efficiently', async () => {
            ...
            mockFs.readdir.mockResolvedValueOnce(largeFileList);
    ...
    describe('error handling', () => {
        ...
        it('should continue processing after individual file errors', async () => {
            const rootPath = '/project';
            mockFs.readdir.mockResolvedValueOnce(['good.md', 'bad.md', 'another.md']);
            mockFs.stat
                .mockResolvedValueOnce({ isDirectory: () => false, isFile: () => true, mtime: new Date(), size: 1000 }) // good.md
                .mockRejectedValueOnce(new Error('EACCES: permission denied')) // bad.md
                .mockResolvedValueOnce({ isDirectory: () => false, isFile: () => true, mtime: new Date(), size: 2000 }); // another.md
            const result = await indexBuilder.buildIndex(rootPath);
...
//# sourceMappingURL=FileIndexBuilder.test.js.map